"""
web_search/train.py — Universal evaluator training contribution.
...
def get_training_pairs(cfg, text_embedder, status_callback=None):
    ...
    storage_dir = OmegaConf.select(
        cfg, "web_search.storage_directory",
    ...
    try:
        ...
        print(f"[web_search/train] DB query failed: {exc}")
        return

    total = len(entries)
    if total == 0:
        print("[web_search/train] No user-rated pages found.")
        return

    print(f"[web_search/train] {total} user-rated pages found.")
    if status_callback:
        status_callback(f"web_search: found {total} user-rated pages.")
    ...
    for i, entry in enumerate(entries):
        ...
        try:
            ...
            print(f"[web_search/train] Error processing {entry.md_file_path}: {exc}")
            continue

        if status_callback and (i + 1) % 10 == 0:
            status_callback(f"web_search: embedded {i + 1}/{total} pages...")
